...
public class TypeServiceImp implements TypeService {
	...
	public void delete(Integer uuid) {
		sessionFactory.getCurrentSession().delete(
				sessionFactory.getCurrentSession().load(Type.class, uuid));
	}

	@Transactional(propagation = Propagation.NOT_SUPPORTED)
	public Type find(String uuid) {
		return (Type) sessionFactory.getCurrentSession().get(Type.class, uuid);
	...
	public Type find(Type model) {
		try {
			List<Type> list = getHibernateTemplate().findByExample(model);
			if (list.size() > 0) {
				return list.get(0);
			}
	...
	public Page findByPage(Page page, Map paramsMap) {
		...
		sb.append("from Type u where 1=1");
		if (paramsMap.get("typename") != null&& !"".equals(paramsMap.get("typename"))) {
			sb.append(" and u.title like '%" + paramsMap.get("typename")+ "%'");
		}
